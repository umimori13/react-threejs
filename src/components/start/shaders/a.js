...
scene.add(particles)
var dotMap = new THREE.TextureLoader().load(
    'https://s3-us-west-2.amazonaws.com/s.cdpn.io/127738/dotTexture.png',
)
var glowMap = new THREE.TextureLoader().load(
    'https://s3-us-west-2.amazonaws.com/s.cdpn.io/127738/glow.png',
)
var pathPoints = [
    [935, 0],
    [1287, 251],
...
function createTube() {
    ...
    for (i = 0; i < opts.segments; i++) {
        ...
        for (var j = 0; j < opts.radiusSegments; j++) {
            ...
            var noiseIndex =
                ((noise.simplex3(p.x * 0.04, p.y * 0.04, p.z * 0.04) + 1) / 2) *
                360
            ...
            geom2.colors.push(
                new THREE.Color('hsl(' + noiseIndex + ',80%,50%)'),
            )
    ...
    for (var i = 0; i < 4000; i++) {
        ...
        geom.colors.push(
            new THREE.Color(
                'hsl(' + Math.floor(Math.random() * 80 + 150) + ',80%,50%)',
            ),
        )
        geom.vertices.push(vertex)
    }
